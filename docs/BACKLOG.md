...
- **`e2e/tests/runner.spec.ts` is flaky on CI** — `expect(tv.locator('.tvtimer')).toBeVisible({timeout: 15000})` at line 50, i.e. the TV never receives the SSE push that starts the giant clock. Failed 3 times across 2 unrelated dependency PRs on 2026-07-27 (stripe-java and nginx, neither of which touches that path), and passed on re-run each time; `main` passes it consistently, and it passes locally even on the nginx bump. So it is a timing sensitivity on slower CI runners, not a product bug — but it costs a re-run every few PRs and, worse, it trains people to re-run red pipelines without reading them. Fix by waiting on the SSE state rather than the rendered element, or by raising just that timeout with a comment saying why.
