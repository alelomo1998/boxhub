...
| 4.5 | **M16a** plan entitlement model | The schema half of M16, **pulled forward ahead of M23**: eight optional limits (entries and cancellations × day/week/month/term) replacing today's single `weekly_class_limit`, plus an append-only usage ledger. Backend only. Pulled forward because M23–M26 build the screens that edit plans, and landing the model after them means rewriting them. Shipped with a per-box cancellation policy the spec did not have, because the spec's fixed late-cancel rule was unreachable in the real engine. | ✅ **done** |
