...
| 5 | **M23** app entry & shells | The container, shaped as its own object before anything is put in it: the **boxless shell**, the box switcher for someone holding several, and how a person moves between a box, another box, and no box. Today all three shells assume a box. **Ships sketches you can look at at 375 and 1440 before anything is built** — M13e's lesson, *"I shaped the sections and not the structure."* | ✅ **done** |
| 6 | **Analytics brief** | **A written brief, not a build**, and moved early on purpose. Asks per role what each stats surface must answer — and **audits whether Phase 1 ever recorded the data**; any migration it forces is a named miss, and a migration is cheap now and ruinous at #27. Owns two questions nothing else does: **"revenue" must be defined before it is charted** (comped subscriptions carry no `payment` row) and **the categorical chart palette does not exist** — one accent and three semantic hues that already mean something, so a five-series chart in volt/green/orange/red tells the reader one series is an error. M17c needs that palette. **Written 2026-08-27: `docs/superpowers/specs/2026-08-27-analytics-brief.md`** — 10 named misses, 6 forced migrations, the revenue definition, and the `--cat-1..6` chart palette. | ✅ **done** |
| 6.5 | **M39** analytics foundations | **The implementation the analytics brief forced**, and the reason it was moved early. **One Flyway pass, six changes:** an append-only `membership_event` table (**without which LEG and churn cannot be computed at all**), `payment.settled_at`, a `refund` table plus `charge.refunded` on the webhook, an explicit `subscription.kind` killing the `"Comped"` string test, `boxes.currency`, and `class_sessions.ran_by_membership_id`. **Two of the six only capture data going FORWARD** — an event nobody recorded is unrecoverable — which is what makes this urgent rather than tidy. Also fixes **D-1**, the live path where editing a class's programming deletes every score logged against it. Backend only, no new screens. `docs/superpowers/specs/2026-08-27-analytics-brief.md` §5. | ✅ **done** |
...
| 11 | **M14c-b** library, benchmarks & types | **The Benchmarks page is deleted** — the WOD library gains a history tab and benchmarks become library entries flagged as such (tour decision 6). **The Types page becomes admin-only** and moves to M15b (tour decision 5 — *"we are permitting our workers to change the structure of the classes? i dont think so"*). Removal of the duplicate function.  Also ended up giving every class calendar a month/year jump and **past days** (a past class keeps only Check-in for coaches and reads Finished / Attended for athletes), and fixed a live overbooking bug: **a check-in freed the athlete's place**, because capacity counted BOOKED only. | ✅ **done** |
| 12 | **M17a** athlete: home, book, class detail — **and the shared class row** | The daily surface. Carries the filed defect at `home.page.ts:38` — the next-booking card links to `/athlete/book` instead of `/athlete/class/:id`, which already exists. **Owns the class row/card as ONE shared component across athlete Book and coach Classes, the actions differing by role** (user-ruled 2026-09-06): M14b gave both screens the week strip but left their rows alone — *"the component owns the strip, not the rows"* — which had left the coach row with no owning milestone. Implements `design-ref/screens/booking-screen-example.webp`, the binding card shape that **nothing implements today**. | |
...
## Phase B — ship it (5 milestones)

| | Milestone | What it actually does | Status |
|---|---|---|---|
| 35 | **M38** scale readiness | **A brief, not a build.** Makes performance visible and gated before there is traffic to be surprised by. The verification effort is concentrated where defects are cheap and reversible (a token, a redeploy); query behaviour under real multi-tenant volume has no gate at all. Adds **query-count tests on the hot paths** (an index does not save you from N+1 — the query is fast and there are four hundred of them), **one load scenario** (the class-opening rush, which M28 names as a rate-limit task but never measures), **slow-query logging and one dashboard**, and a **written p95 budget** per hot endpoint. The schema itself is in good shape — 49 indexes, 25 covering `box_id`, several well-chosen composites — so this is deliberately small. `docs/superpowers/specs/2026-08-27-m38-scale-readiness-brief.md`. | |
| 35b | **M40** write-path limits | **A brief plus a small build.** The companion to M38: M38 makes performance visible, this makes *abuse* visible. `AuthRateLimitFilter`'s write bucket is a four-path allowlist and every bucket is keyed on **IP, not user** — so 47 POST endpoints have 4 rate-limited, and 41 request records have 11 `@Size` caps. A logged-in member can create ~1200 rows/min on any write endpoint (`POST /api/box/lifts` is the worst: ATHLETE-reachable, unbounded, free-text `notes` with no cap), and because a gym shares one NAT IP, doing so can 429 their own coaches mid-class. Deliverable: a **per-membership** limit keyed on `TenantContext` for athlete-reachable writes, `@Size` on every free-text field, and a **standing grep** so a new uncapped write endpoint fails the build the way an unregistered route already does. **Not M28's**: M28 owns *tuning* limits for legitimate load, this owns *coverage*. Not M38's: M38 measures, this bounds. Details and numbers in `docs/BACKLOG.md`. | |
| 36 | **M28** Launch → Production | The deploy, once there is a product to deploy. TLS/HSTS, domain, firewall, SSH hardening, **Postgres backups + a restore drill**, secrets delivery, log retention, CI deploy on green, **real SMTP + SPF/DKIM/DMARC**, error monitoring and uptime, `BOXHUB_COOKIE_SECURE=true`, ToS/privacy/DPA (EU PII — and it must cover M33's import, where the box is controller and rxed is processor), rate limits under a class-opening rush, gym-domain mail, and deleting `/app/dev/components`. **Scope was named and deferred by M12c in July — read that spec first or you will rebuild its work.** **`deploy/deploy.sh` has NEVER successfully run**: M12c added a sentinel guard to a script that has never executed, so expect to debug the script as well as the deploy. The two nginx configs must be unified first — `docker/nginx-tls.conf` is a 154-line hand-maintained copy of `docker/nginx.conf`. | |
| 37 | **M27d** store release | Listings, screenshots, privacy declarations, TestFlight and internal testing, review submission. **Cannot precede M28** — Apple and Google review the app against a real backend, not localhost. Mostly calendar time and fixing rejections. | |
| 38 | **M19** landing site | The public marketing site at `/`, application under `/app`. Built from `docs/POSITIONING.md` §7. Independent of box pages, which stay signed-in-only. The natural forcing point for **icon assets beyond the favicon** — no `apple-touch-icon`, no web manifest, no `theme-color` today, and M27a wants them too. **Decide first what happens to the `oc/m19-landing` branch and its 9 commits.** | |
| 39 | **M20** 2FA / TOTP | Two-factor for box owners and superadmins. Small, independent, no reason to be earlier. | |
