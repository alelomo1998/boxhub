...
- **M16a plan entitlement model (2026-08-22, branch `m16a-entitlement-model`)** — a plan carried one limit; now it carries eight, and consumption is counted from a ledger scheduling cannot touch. Flyway **V28**: eight nullable ints on `plans` (entries and cancellations × day/week/month/term, **NULL = unlimited**, all composing with **AND** — no precedence, no "tightest wins"), three per-box cancellation-policy booleans, and the append-only **`entitlement_usage`** table. `plan.weekly_class_limit` and `plan.entitlement` are **dropped as columns** but **still served**, derived, exactly as M14a kept `wodType` alive — see the shim note below. **Windows anchor to the SESSION being booked, not to `now()`** (`PlanLimits.window`, box timezone, DST-correct), so booking next Tuesday counts against next Tuesday's day and week; anchoring to `now()` would let an athlete drain the wrong week by booking ahead. `total` is scoped to `subscription.current_period_start/end`, so a monthly term resets monthly and an annual one annually with no new date arithmetic and nothing for an admin to reset — a punch-card is just `entries_total = 10` with the other seven null.

  **Why a ledger and not a query over `bookings` — and the spec was wrong about this.** The spec said regeneration deletes bookings and would silently change consumed counts. It does not: `SlotRegenerationService.regenerateFrom` **refuses** a range holding a live booking (409 `RANGE_HAS_BOOKINGS`) and only ever deletes **CANCELLED** rows. So it cannot erase a live entry — but it *can* erase cancellation history and the unrefunded entry behind a late cancel, both of which M16a starts counting. The table is still required, for a narrower and more precise reason than the spec gave. `entitlement_usage.booking_id` carries **no FK** (an FK would either block that delete or cascade the ledger away with it) and `session_start_at` is **copied, not joined**. `EntitlementLimitsTest.regeneratingASlotDoesNotAlterConsumedCounts` is built on a CANCELLED booking for exactly this reason — built on a live one it would prove nothing, since regeneration would simply refuse.

  **Four decisions overrode the spec, all user-stated 2026-08-22.** (1) **A waitlist join consumes an ENTRY at join**, not at promotion — so **promotion writes nothing to the ledger** and can never push anyone past a limit, and `cancel()` stays a pure queue shift. This reversed a rule the old engine encoded: `countInWeek` filtered to BOOKED/CHECKED_IN, so a waitlisted athlete was free and someone at their limit could queue for every class in the week and be promoted past it. `BookingEngineTest.weeklyLimitBlocksAtBoundaryAndIgnoresWaitlist` was renamed and rewritten to pin the new rule. (2) Whether a **waitlist cancellation** spends a CANCELLATION is `boxes.count_waitlist_cancellations`, default false; a waitlist cancel always refunds the entry, because lateness is meaningless for someone who never held a seat. (3) The 409 reason for a blocked booking stays the bare **`LIMIT_REACHED`** — `book.page.ts:166` switches on that exact string and M16a freezes every frontend number. (4) **Late-cancel behaviour is per-box policy, not a fixed rule**, because the spec's version was unreachable: `cancel()` throws `PAST_CUTOFF` for any BOOKED booking past the cutoff, so `was_late = true` was only ever reachable on a waitlist cancel — `BookingCancellationTest:182` says so in its own comment. `boxes.allow_late_cancel` and `boxes.late_cancel_refunds_entry` (both default false = today's exact behaviour) make it real. "How late is late" needed **no new column**: `boxes.cancel_cutoff_min` already exists (V21) and is already PATCHable.

  **The shim, and who owes its deletion.** `entitlement` is served as `UNLIMITED` when all eight limits are null else `WEEKLY_LIMIT`; `weeklyClassLimit` is served as `entries_per_week`; writes accept both. It exists because `plans.page.ts` binds both and `membership.service.ts` types both, and fixing those screens is frontend work M16a forbids. **Whichever milestone rebuilds the plan admin screen (M14b or M17) deletes the shim in `PlanController.PlanDto`, `SubscriptionController.PlanSummaryDto`, and `PlanController.requireWeeklyLimit` with it.** Recorded in `docs/BACKLOG.md`, not left implicit. Note `requireWeeklyLimit`: the plan claimed `resolveEntitlement` was pure redundancy and could be deleted whole. It could not — `plans.page.ts` can post `entitlement: 'WEEKLY_LIMIT'` with an empty limit input, which without the check becomes a silently-unlimited plan named as if it were limited. An executor caught that by refusing to commit around a red test rather than editing the test.

  **Gates, all measured**: backend **535/0/0** (from 511); Karma **419**, e2e **67**, visual **31 passed / 0 dirty**, eight §8.1 greps **0 bytes**, `git diff main -- frontend e2e` **0 bytes** — the frozen frontend numbers *are* this milestone's own scope test, and they held. `AuthzConformanceTest` untouched (no new routes); `runAsRoot` in controllers still zero. **Negative control run on all eight new integration tests, and it earned its place**: the `lastWeeksBooking` mutation was a **false negative** as the plan specified it — the fixture sits a year ahead of `now()`, so a `now()`-anchored window contained neither session and the test stayed green under the very mutation it was meant to catch. A third same-week assertion is what actually distinguishes session-anchored from now-anchored. Two other plan claims were also false and are corrected in the plan file: seven tests were really eight, and `BookingRepository.countInWeek` is **not** dead (`HomeController:84` still counts weekly check-ins with it).
