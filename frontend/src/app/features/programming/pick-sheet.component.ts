...
import { MOVEMENT_UNITS } from './programming.service';
...
@Component({
      ...
      @if (step() === 'search') {
        ...
        <div class="create-step" data-testid="pick-create-step">
          ...
          <div class="create-actions">
            <bh-button variant="ghost" testId="pick-create-back" (click)="backToSearch()">
              <span i18n="@@pickSheet.create.back">Back</span>
            </bh-button>
            <bh-button variant="strong" size="lg" testId="pick-create-confirm"
  ...
  styles: [`
    ...
    .create-actions { display: flex; gap: var(--sp-3); margin-top: var(--sp-3); }
    .create-actions bh-button:last-child { flex: 1; }
  `],
})
export class PickSheetComponent {
  ...
  create = output<{ name: string; units: string[]; loadable: boolean }>();
  closed = output<void>();
  ...
  confirmCreate() {
    this.create.emit({ name: this.term().trim(), units: this.createUnits(), loadable: this.createLoadable() });
  }
